...
require([
    ...
    const templateGenerator = Handlebars.compile(
    ...
    const index_to = location.pathname.indexOf('docs/') - 1
    ...
    apiGroups.forEach(function (groupEntry) {
        ...
        api.forEach(function (entry) {
            if (groupEntry === entry.group) {
                ...
                if (apiProject.url) {
                    fields.article.url =
                        docs_location + apiProject.url + fields.article.url
                }
                try {
    ...
    function sortFields(fields_object) {
        $.each(fields_object, function (key, fields) {
            ...
            const max_dot_count = Math.max.apply(
                null,
            ...
            for (var dot_count = 1; dot_count <= max_dot_count; dot_count++) {
                reversed.forEach(function (item, index) {
                    ...
                    if (parts.length - 1 == dot_count) {
                        const fields_names = fields.map(function (item) {
                            return item.field
                        })
...
function on_click_burger(item) {
    ...
    if (burger_side_showing) {
        ...
        $('.sidenav-search').each(function () {
            $(this).removeClass('navbar_hide')
        })
